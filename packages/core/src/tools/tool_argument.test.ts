...
import { type ToolArgument, validateToolArgument } from "./tool_argument"; // Adjust path to your file

describe("validateToolArgument", () => {
    // --- PRIMITIVES ---

    it("should validate strings and check enums", () => {
        const stringArg: ToolArgument = {
            type: "string",
    ...
    it("should validate numbers and reject NaN/Infinity", () => {
        const numArg: ToolArgument = {
            type: "number",
            description: "a float/double",
        };
    ...
    it("should validate arrays recursively", () => {
        ...
        expect(validateToolArgument(stringArrayArg, ["ai", "mcp", "ts"])).toBe(
            true,
        );
        expect(validateToolArgument(stringArrayArg, ["ai", 123])).toBe(false); // bad item
        expect(validateToolArgument(stringArrayArg, "not an array")).toBe(
            false,
        );
    });
    ...
    describe("objects and nesting", () => {
        ...
        it("should allow omitting optional fields", () => {
            ...
            expect(validateToolArgument(userSchema, missingOptional)).toBe(
                true,
            );
        });
        ...
        it("should fail if required fields are missing", () => {
            ...
            expect(validateToolArgument(userSchema, missingRequired)).toBe(
                false,
            );
        });
        ...
        it("should fail if nested property validation fails", () => {
            ...
            expect(validateToolArgument(userSchema, badPropertyType)).toBe(
                false,
            );
        });
        ...
        it("should handle additionalProperties restriction cleanly", () => {
            const strictSchema: ToolArgument = {
                ...
                properties: {
                    allowed: { type: "boolean", description: "flag" },
                },
                additionalProperties: false,
            };

            expect(validateToolArgument(strictSchema, { allowed: true })).toBe(
                true,
            );
            expect(
                validateToolArgument(strictSchema, {
                    allowed: true,
                    rogueKey: "hack",
                }),
            ).toBe(false);
        ...
        it("should handle deeply nested arrays of objects", () => {
            ...
            expect(validateToolArgument(complexSchema, validPayload)).toBe(
                true,
            );
            expect(validateToolArgument(complexSchema, invalidPayload)).toBe(
                false,
            );
        });
    });
